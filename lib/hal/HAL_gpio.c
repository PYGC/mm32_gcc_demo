...
void GPIO_DeInit(GPIO_TypeDef* GPIOx)
{
    switch (*(u32*)&GPIOx) {
#if defined(GPIOA)
        case GPIOA_BASE: COMMON_DisableIpClock(emCLOCK_GPIOA);	break;
#endif
#if defined(GPIOB)
        case GPIOB_BASE: COMMON_DisableIpClock(emCLOCK_GPIOB);	break;
#endif
#if defined(GPIOC)
        case GPIOC_BASE: COMMON_DisableIpClock(emCLOCK_GPIOC);	break;
#endif
#if defined(GPIOD)
        case GPIOD_BASE: COMMON_DisableIpClock(emCLOCK_GPIOD);	break;
#endif
        default: break;
    }
}
...
{
#if defined(GPIOC)
	if ((u32)GPIOx == GPIOC_BASE) {
		GPIO_checkBKP();
	}
#endif

	u32 data = (pInitStruct->GPIO_Mode & 0x10) ?
			   (pInitStruct->GPIO_Mode & 0x0F) + pInitStruct->GPIO_Speed :
			   (pInitStruct->GPIO_Mode & 0x0F);

	for (u8 i = 0; i < 16; i++) {
		if ((pInitStruct->GPIO_Pin) & (1 << i)) {
			__IO u32* cr  = (i < 8) ? &GPIOx->CRL : &GPIOx->CRH;
			u8		  idx = (i < 8) ? i * 4 : (i - 8) * 4;
			*cr = (*cr & ~(0xF << idx)) | (data << idx);
		}
	}
